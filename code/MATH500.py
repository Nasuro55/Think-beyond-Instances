        ...
        with open(dataset_file, 'r', encoding='utf-8') as f:
            for line in f:
                try:
                    ...
                    if 'level' in data and isinstance(data['level'], int): data['level'] = str(data['level'])
                    if 'subject' in data: data['type'] = data['subject']
                    test_data.append(data)
                except: continue
    except: return create_sample_dataset()
...
def extract_steps_with_quad_cards(answers: list[str], question: str) -> list[dict]:
    ...
    messages = [
        {"role": "system", "content":
            ...
            "FORMAT:\n"
            "Step 1: || Card A: [Logic 1] Math: [Equation/Result] || Card B: [Logic 2] Math: [Equation/Result]|| Card C: [Logic 3] Math: [Equation/Result] || Card D: [Logic 4] Math: [Equation/Result]\n"
            "Step 2: || ...\n"
        },
        {"role": "user", "content": 
            f"Question: {question}\n\nReference Solution:\n{best_answer}\n\n"
...
def main():
    ...
    # Select subset for testing (Restoring user's specific range)
    problems_to_run = records[360:501] 
