...
print("Initializing Qwen client with Local Path...")
...
# ============================================================================
# 3. DATASET LOADING (AIME25 Logic)
# ============================================================================

def create_sample_dataset():
    print("Using built-in sample problems for testing...")
    sample_problems = [
        {"problem": "If $3x + 7 = 22$, what is the value of $x$?", "solution": "5", "answer": "5", "level": "4", "subject": "Algebra"},
    ]
...
def load_real_math_dataset():
    ...
    if not os.path.exists(dataset_file):
        print(f"File {dataset_file} not found. Using sample.")
            ...
            for line in f:
                try:
                    ...
                    if 'answer' in data:
                        ...
                        data['final_gt'] = parse_numeric_answer(data['solution'])
                    else:
                        data['final_gt'] = "Unknown"

                    if 'subject' in data: data['type'] = data['subject']
...
def generate_baseline_answer(question: str) -> str:
    messages = [
        {"role": "system", "content": "You are a math solver. Provide clear reasoning and place the final numeric answer inside \\boxed{} at the very end."},
...
# ============================================================================
# 5. PIPELINE: MUTATION & VERIFICATION LOOP (ROBUST MINERVA LOGIC)
# ============================================================================
...
def main():
    ...
    for item in tqdm(problems_to_run, desc="Evaluating"):
        ...
        current_idx = max(index, 1)
        base_acc = 100 * base_correct / current_idx
        pipe_acc = 100 * pipe_correct / current_idx
        
        print(f"Baseline Correct: {base_correct}/{index} ({base_acc:.1f}%)")
        print(f"Pipeline Correct: {pipe_correct}/{index} ({pipe_acc:.1f}%)")
        print(f"Net Gain: +{pipe_correct - base_correct}")
        
        with open(f"{results_dir}/detailed_results.txt", "a", encoding="utf-8") as out:
            out.write(f"Q{index} [{item.get('subject','')}]: GT={gt} | Base={base_ans} ({is_base_correct}) | Pipe={pipe_ans} ({is_pipe_correct}) [{pipeline_note}]\n")
